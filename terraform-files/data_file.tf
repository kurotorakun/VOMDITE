...
data "local_file" "ansible_id_rsa" {
    filename   = "/home/abc/.ssh/id_rsa"
}

# Ansible service public key
data "local_file" "ansible_id_rsa_pub" {
    filename   = "/home/abc/.ssh/id_rsa.pub"
}
...
data "template_file" "ans_userDefault" { 
  ...
  vars         = {
    HOSTPUBKEY      = data.local_file.host_pubkey.content                       # Use this only for simple files
    ANSIBLEKEY      = base64encode(data.local_file.ansible_id_rsa.content)      # BEST PRACTICE for files to be
    ANSIBLEPUBKEY   = base64encode(data.local_file.ansible_id_rsa_pub.content)  #   written in the host
  }
}
