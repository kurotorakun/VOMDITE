...
resource "null_resource" "deploy_balancer" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      ssh -o StrictHostKeyChecking=no -t ${var.linux_username}@${var.VMNetwork_CIDR}.${var.ansible_address} "rsync -r --rsync-path='sudo rsync' --include '*.conf' --exclude '*.yml' '/home/ubuntu/ansible-balancer-deploy/' ${var.linux_username}@${var.DC_uplink_CIDR}.${var.balancer_address}:/home/ubuntu/balancer/"
